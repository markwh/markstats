...
test_that("imputation of left-right data works", {
  ...
  expect_less_than(abs(mean(randlr(normlr, "norm")$imputed) - 100), 0.02)
  expect_less_than(abs(sd(randlr(normlr, "norm")$imputed) - 1), 0.02)
  
  expect_less_than(abs(median(log(randlr(lnormlr, "norm")$imputed)) - 2), 0.2)
})